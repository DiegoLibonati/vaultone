...
6. **`publish-release`** — uses `python-semantic-release`'s publish action to create the GitHub Release for the new tag, attaching the Windows `.exe` and the generated changelog section as the release notes.
...
| `perf:` / `perf(scope):` | **PATCH** | `perf: cache mic icon lookup` |
| `refactor:`, `docs:`, `build:`, `ci:`, `chore:`, `style:`, `test:` | **none** | `refactor: extract audio helper` |
